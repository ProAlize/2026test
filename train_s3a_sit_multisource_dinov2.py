...
"""
S3A training script for SiT on ImageNet-style data.

S3A (Stage-Conditioned Source-and-Structure Alignment) keeps the SiT backbone
...
Key differences from the DiT version:
  1. Uses SiT transport/interpolant framework instead of DDPM diffusion
  2. Continuous time t ∈ [0,1] (t=0 noise, t=1 clean; opposite of DDPM)
  3. Cross-timestep self-source: t_ema = min(1, t + k) instead of max(0, t - k)
  4. Timestep weighting uses flipped t: t_for_weight = 1 - t_continuous
"""
...
# SiT uses external model definitions and transport framework.
# Ensure the SiT repo (willisma/SiT) is on the Python path.
...
def get_diff_timestep_weight(
    ...
    For SiT: T is ignored. t is already continuous in [0, 1] and has been
    flipped so that t_norm=0 corresponds to clean data (matching DDPM
    convention used by REPA-style weighting).
    """
    t_norm = t.float()  # already in [0, 1] for SiT (flipped: 0=clean, 1=noise)
...
def main(args):
    ...
    if args.s3a:
        ...
        s3a_layer_indices = parse_layer_indices(args.s3a_layer_indices, sit_depth)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (images, y) in enumerate(loader):
            ...
            if train_steps == 1 and rank == 0:
                ...
                if t_continuous is not None:
                    logger.info(f"  [step=1] t_cont (first 8) : {t_continuous[:8].tolist()}")
                    logger.info(f"  [step=1] t_s3a  (first 8) : {t_for_s3a[:8].tolist()}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "SiT + S3A training with DINOv2 teacher.\n"
            "S3A: multi-layer multi-source alignment branch with dynamic routing.\n"
            "Uses SiT transport/interpolant framework instead of DDPM diffusion.\n"
    ...
    parser.add_argument(
        ...
        help="SiT interpolation path type. REPA default: linear.",
    )
    parser.add_argument(
        "--prediction", type=str, default="v",
        choices=["v", "x", "noise"],
        help="SiT prediction target. REPA default: v (velocity).",
    )
    parser.add_argument(
        "--loss-weight", type=str, default=None,
        choices=[None, "velocity", "likelihood"],
        help="SiT loss weighting. REPA default: None (uniform).",
    )
    parser.add_argument(
        "--train-eps", type=float, default=None,
        help="Training time lower bound (small epsilon to avoid t=0).",
    )
    parser.add_argument(
        "--sample-eps", type=float, default=None,
        help="Sampling time lower bound.",
    )
    parser.add_argument(
