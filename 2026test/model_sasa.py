...
# SASA-A-cosine keeps the DiT backbone unchanged.
# This file is a standalone copy for train_sasa.py so the experiment can run
# without modifying the original models_2.py.
...
class DiT(nn.Module):
    ...
    def forward_features(self, x, t, y, token_layer=None):
        ...
        Args:
            ...
            y: (N,) class labels
            token_layer: int or None
                - if None: return the last block output tokens
                - else: return tokens after blocks[token_layer]

        Returns:
            tokens: (N, T, D)
            c: conditioning embedding (N, D)
        """
